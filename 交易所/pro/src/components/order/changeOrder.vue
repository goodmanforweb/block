<style lang=less>
	...
	@import './changeOrder.less';

	.change_order_wrapper .relatives input {
		background: #171B2B;
		color: #fff;
	}

	.change_order_wrapper .relatives span {
		color: #fff;
	}

	.change_order_wrapper .relativesdd .spanss{
		color: #fff;
	}
	.change_order_wrapper {
		...
		.dialog_set .dialog-footer button,
		.dialogwrappers .el-button {
			border: none;
			background-color: #FFA517 !important;
			color: #FFF;
		}

		.dialogwrappers .dialog-footer .el-button--primary {
			background: #2A64F3 !important;
			border: none;
			color: #FFF;
		}
		.el-table__header {
...
<template>
	<div class="change_order_wrapper">
				...
				<el-tabs v-model="activeName" @tab-click="handleClick">
					...
					<el-tab-pane :label="$t('ggOrder.now[0]')" name="first" class="historyWt">
						...
						<el-table :data="requestData3" style="width: 100%;" height="600"
							...
							<el-table-column prop="hold_no" :label="$t('ggOrder.now[1]')" width="130">
							</el-table-column>
							...
							<el-table-column :label="$t('ggOrder.now[6]')" width="100">
								<template slot-scope="scope">
							...
							<el-table-column :label="$t('ggOrder.now[10]')" width="100">
								<template slot-scope="scope">
									{{Number(scope.row.poit_loss).toFixed(2)}}
								</template>
							</el-table-column>
							<el-table-column prop="dayfee" :label="$t('ggOrder.now[11]')" width="100">
							</el-table-column>
							<el-table-column :label="$t('ggOrder.now[12]')" width="120">
								<!-- <template slot-scope="scope">
							...
							<el-table-column :label="$t('ggOrder.main[1]')" width="120">
								<!-- <template slot-scope="scope">
									<div v-if="scope.row.fdyk == 0 ">— —</div>
									<div v-else>{{ scope.row.fdyk }}</div>
								</template> -->
							...
							<el-table-column prop="id" :label="$t('ggOrder.main[26]')" width="120">
								<template slot-scope="scope">
									...
									style="background: #FFA517; border: none; color: #FFF;">
										{{$t('ggOrder.news[10]')}}</el-button>
									<el-button type="primary" 
									class="todos" 
									@click="deleteRow(scope, requestData2)"
									style="background: #2A64F3; border: none; color: #FFF;">
					...
					<el-tab-pane :label="$t('ggOrder.historyOrder[0]')" name="fourth" class="histroyDeal historyWt">
						...
						:header-cell-style="{background:'#282C3D',color: '#AEAFB3'}">
							<el-table-column prop="tran_no" :label="$t('ggOrder.historyOrder[1]')" width='130'>
							</el-table-column>
							...
							<el-table-column prop="otype" :label="$t('ggOrder.historyOrder[3]')" width="80">
								<template slot-scope="scope">
									<div v-if="scope.row.otype == 1 " class="green">{{$t('ggOrder.main[13]')}}</div>
									<div v-else class="red">{{$t('ggOrder.main[14]')}}</div>
								</template>
							</el-table-column>
							<el-table-column :label="$t('ggOrder.historyOrder[4]')" width="100">
									<!-- <template slot-scope="scope">
							...
							<el-table-column prop="sellprice" :label="$t('ggOrder.historyOrder[5]')" width="100">
								<template slot-scope="scope">
									{{Number(scope.row.sellprice).toFixed(2)}}
								</template>
							</el-table-column>
							<el-table-column prop="buynum" :label="$t('ggOrder.historyOrder[6]')" width="60">
							</el-table-column>
							<el-table-column prop="sxfee" :label="$t('ggOrder.historyOrder[7]')" width="100">
								<!-- <template slot-scope="scope">
									<div>{{ $public.timestampToTime(scope.row.selltime) }}</div>
								</template> -->
							</el-table-column>
							...
							<el-table-column :label="$t('ggOrder.historyOrder[10]')" width="120">
								<template slot-scope="scope">
									{{Number(scope.row.poit_win).toFixed(2)}}
								</template>
							</el-table-column>
							<el-table-column :label="$t('ggOrder.historyOrder[11]')" width="120">
								<template slot-scope="scope">
									{{Number(scope.row.poit_loss).toFixed(2)}}
								</template>
							</el-table-column>
	...
	export default {
		...
		methods: {
			woyaohuilv() {
				...
				this.loading = true
				this.$http.get(this.$http.huilv,{params:{account:sessionStorage.getItem('account')}}).then(function (res) {
					_this.loading = false
					if(res.data.status == 200){
						
						// _this.fygz = _this.$t('currlanguage')=="cn"? res.data.data.fygz : res.data.data.data
						_this.fygz = res.data.data.value
						_this.gethuilv=true
					}else{
						_this.$message({
							showClose: true,
