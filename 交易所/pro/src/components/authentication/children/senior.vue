...
<template lang="html">
    <div class="identity">
            ...
            <div class="from">
                <el-form :model="form" :rules="rules2" ref="form" label-width="100px" label-position="left">
                    <div v-if='form.type == "1"' class="imgWarp">
                      <div class="efi--roleCard1">
                         <el-form-item class='authenTication' prop="realname">
                        <div class="name1">
                            <span>{{$t("identityAu.mian[0]")}}</span>
                          <el-input  v-model="form.realname" :placeholder="$t('real_name.verification[9]')"></el-input>
                        </div>
                        </el-form-item>
                         <el-form-item  prop="card_num">
                         <div class='identity1'>
                            <span>{{$t("identityAu.mian[1]")}}</span>
                          <el-input  v-model="form.card_num"  style="width:540px;background-color:#181b29;"  :placeholder="$t('real_name.verification[10]')"></el-input>
                        </div>
                        </el-form-item>
                      </div>
        ...
        <section class="payment">
            <div class="but">
                <el-button :loading="loading" @click="onSubmit">{{$t('identityAu.mian[16]')}}</el-button>
            </div>
        </section>
    </div>
</template>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      _this.$refs["form"].validate(valid => {
        ...
        var sendData = {
          card_num: _this.form.card_num, //证件号
        ...
        _this.loading = true
        _this.$http.post(_this.$http.set_sm, sendData)
          .then(function(response) {
            _this.loading = false
            if (response.data.status == "200") {
