  ...
  <div class="chong_bit_wrapper">
    <div class="chong_bit_container">
      <div class="chong_bit_header">
        <div class="title">
          <span>{{$t('moneyRecord.main[0]')}} >{{$t('zeusBi.cb[1]')}}</span>
          <!-- <span @click="$router.push('/aKeyBuy')" class="right">{{$t('zeusBi.cb[1]')}}</span> -->
        </div>
      </div>
      <div class="chong_bit_code">
        
          <img :src="$http.bases + requestData.qrc" style="width:98px;height:98px;" alt="">
          <!-- <template slot-scope="scope">
            <img :src="scope.row.headImg" width="98px" height="98px" alt="">
      
          </template> -->
        
        <p>{{$t('zeusBi.cb[17]')}}</p>
        <a id="download"  download="123.png"   :href="$http.bases + requestData.qrc">{{$t('chongBit.main[16]')}}</a>
      </div>
      <div class="chong_top">
