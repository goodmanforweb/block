<style lang=less>
	...
	.exchange {
		...
		.makeAbargin .deepMerge .el-input__suffix-inner {
				...
				<div class="klinediv">
					...
					<div class="hangqing">
						<div class="hangqing_nav">
							<div class="item">{{ $t('index.myPrice[0]') }}</div>
						</div>
						<div class="hangqing_list">
							<div class="hangqing_list_item" 
							v-for="(item,index) in Marketdatamain" :key="index"
							@click="handleCommand(item.code,item.price)"
							>
								<div class="coin">
									<img :src="coinImg(item.name)" alt="">
									<div class="coinInfo">
										<div :class="'coinInfo_name'">{{item.name}}</div>
										<div :class="'coinInfo_cny'+' cny-'+item.code">≈{{item.cnyPrice && Number(item.cnyPrice).toFixed(2)}}</div>
									</div>
								</div>
								<div class="coinPrice">
									<div :class="'coinPrice_now'+' price-'+item.code">{{ item.price && Number(item.price).toFixed($public.savePoint(item.code)) }}</div>
					...
					<div style="flex-grow: 1">
						...
						<section class="kine" v-if="klinetab == $t('index.KLine[13]')">
							<iframe id="fram" :src="'./static/index.html?timers=5&code=' + marketPurchase.code + '&last_price=' + urlPrice" width="100%"
								height="100%;"></iframe>
						</section>
						<section class="kine" v-else>
							<iframe id="fram" :src="'./static/indexsd.html?timers=5&code=' + marketPurchase.code + '&last_price=' + urlPrice" width="100%"
								height="100%;"></iframe>
						</section>
					</div>
				...
				<section class="HistoricalEntrustment mt10">
					...
					<div>
						<div :class="$t('currlanguage')=='cn'? 'tojingtotles' : 'tojingtotles modelEn'">
							...
							<span style="margin-left: 30px">{{$t('index.KLine[11]')}}: {{tongjiData.yingkui}}</span>
						</div>
						<ggOrder 
						:changeactive="updateOrder" 
						:part="true" 
						@changeData="listenToMyBoy" 
						ref="offTime1" 
						:price="marketPurchase.PriceData" 
						:Code="marketPurchase.code"></ggOrder>
					</div>
				</section>
			</div>
				...
				<div class="makeAbargin">
								...
								<el-tab-pane>
									...
									<div class="transaction_main" v-if="PurchaseData">
										...
										<div class="relativebs">
											<div class="new_price">
												<!-- {{$t('index.header[1]')}}: -->
												<span class="cnyprice rise" v-if="Marketdataheader.change>0">
													{{Number(Marketdataheader.price).toFixed($public.savePoint(Marketdataheader.code))}}
													<i>↑</i>
												</span>
												<span class="cnyprice fall" v-else>
													{{Marketdataheader.price}}
													<i>↓</i>
												</span>
												<span
													class="cny">￥{{Number(Marketdataheader.cnyPrice).toFixed(2)}}
												</span>
											</div>
					...
					<div class="makeAbarginItem makeAbarginRight">
						<p class="makeAbarginItemP topP">
							<span>{{$t('index.KLine[16]')}}(USDT)</span>
				...
				<section class="transaction mt10">
					<div class="transaction_box">
						<el-tabs v-model="activeName">
							<el-tab-pane :label="$t('index.KLine[3]')" name="first">
							...
							<el-tab-pane :label="$t('index.KLine[2]')" name="second">
...
<script>
	...
	import ggOrder from "../order/changeOrder.vue"; //委托
	...
	export default {
		data() {
			return {
				// urlPrice: this.$route.query.urlPrice ? Number(this.$route.query.urlPrice).toFixed(this.$public.savePoint(this.$route.query.code)) : '',
				urlPrice: this.$route.query.urlPrice,
				childNum: '',
				updateOrder:false,
				deepMergeModel:4,
				deepMerge:[
		...
		// beforeCreate() {
		// 	// 接收首页路由传送价格
		// 	if (this.$route.query.urlPrice) {
		// 		console.log(this.$route.query.urlPrice)
		// 		this.urlPrice = Number(this.$route.query.urlPrice).toFixed(this.$public.savePoint(this.$route.query.code))
		// 	}
		// },
		methods: {
			transDeepMerge(val){
			...
			// 
			handleCommand(command,price) {
				var _this = this;
				_this.urlPrice = Number(price).toFixed(_this.$public.savePoint(command))
			...
			listenToMyBoy(state) {
				...
				this.getdatamain(state);
				this.amount(); //个人资产数据
			},
			getdepth() {
				this.$http.get(this.$http.getdepth, {params: {code :this.marketPurchase.code}}).then((r)=>{
					if(r.data.status == 200){ 
						this.PurchaseData = JSON.parse(JSON.stringify(r.data.data[0])) //join
						this.PurchaseData.sell = r.data.data[0].asks.reverse();
						this.PurchaseData.buy = r.data.data[0].bids;
					}else{ 
			...
			MainDatapush(code) {
				...
				_this.MainData.onmessage = function (evnt) {
					...
					// if(_this.$route.path == '/exchange'){
						cnyNode.innerHTML = `≈${Number(datanum.cnyPrice).toFixed(2)}`
						priceNode.innerHTML = `${Number(datanum.price).toFixed(_this.$public.savePoint(datanum.code))}`
			...
			SalePriceDatapush(type) {
				...
				connet.SalePriceData.onmessage = function (evnt) {
					var datanum = JSON.parse(evnt.data);
					connet.PurchaseData = JSON.parse(JSON.stringify(datanum)) //join
			...
			// 计算背景宽度 当前数值/当前数组中最大数值 
 			renderBgWdith(array, num) { // 数组，当前值
				var max;
		...
		mounted() {
			...
			if (_this.$route.query.code) {
				_this.handleCommand(_this.$route.query.code, this.$route.query.urlPrice);
			}
		},
		components: {
			Purchase: Purchase,
