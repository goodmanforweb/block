						...
						<div class="sellBuy_title">
							...
							<div class="form_warp">
								...
								<el-form :label-position="labelPosition" :model="sellData" status-icon :rules="rules2" ref="sellData"  class="demo-ruleForm">
									...
									<el-form-item :label="$t('sell.main[0]')" prop="price">
										<el-input v-model.number="sellData.price" auto-complete="off" type="number"></el-input>
									</el-form-item>
					
									<el-form-item :label="$t('sell.main[1]')" prop="trans_num">
										<el-input v-model.trans_number="sellData.trans_num" auto-complete="off" type="number"></el-input>
					
									</el-form-item>
									<div class="limit">
										<el-form-item :label="$t('sell.main[2]')" prop="max_price">
											<el-input type="number" v-model.number="sellData.max_price" auto-complete="off" :placeholder="$t('sell.main[3]')" ></el-input>
											<span class="cny">CNY</span>
										</el-form-item>
					
										<el-form-item label="  ." prop="min_price">
											<el-input type="number" v-model.number="sellData.min_price" auto-complete="off" :placeholder="$t('sell.main[4]')" ></el-input>
											<span class="cny">CNY</span>
										</el-form-item>
									</div>
									<el-form-item :label="$t('moneyAddre.main[4]')" prop="notes">
										<el-input v-model="sellData.notes" auto-complete="off" type="text"></el-input>
									</el-form-item>
									...
									<el-form-item>
										...
										<ul class="lis_warp">
											<li v-for="(v,k) in pay_list" v-show="v.status !== 1" v-loading="loadings">
												...
												<el-checkbox v-model="sellData.pay_paypal" :true-label='1' false-label='0' @change='changeAct(v.type)' v-else-if="v.type == 'paypal'"></el-checkbox>
					
											</li>
										</ul>
										<router-link v-if="pay_list.length < 3" to="/security">
											<div class="add_pay">
												<p>{{$t('sell.main[7]')}}</p>
											</div>
										</router-link>
									...
									<el-form-item>
										<el-button type="primary" @click="submitForm('sellData')" :loading="loading">{{$t('sell.main[8]')}}</el-button>
									</el-form-item>
								</el-form>
							</div>
							<div class="sellBuy_list">
								...
								<div class="sellBuy_title_warp">
									<div class="sellBuy_nav">
										<span class="dingdan">{{$t('sell.main[10]')}}</span>
										<span>{{$t('sell.main[11]')}}</span>
										<span>{{$t('sell.main[12]')}}</span>
										<span>{{$t('sell.main[13]')}}</span>
										<span>{{$t('sell.main[14]')}}</span>
										<span>{{$t('sell.main[15]')}}</span>
										<span class="todo">{{$t('sell.main[16]')}}</span>
									</div>
									<p class="no_list" v-show="sell.sell_list.length <= 0">{{$t('sell.main[17]')}}</p>
									<ul class="sellBuy_lis_warp" v-show="sell.sell_list.length > 0">
										<li class="sellBuy_lis" v-for="(v,k) in sell.sell_list">
											...
											<p>
												<span class="todo" v-if="v.status == '1'" @click="pmmaRev(v.id)">{{$t('sell.main[18]')}}</span>
		...
		data() {
			...
			var trans_num = (rule, value, callback) => {
				if(!_this.$public.Ints(value)) {
					return callback(new Error(_this.$t('sell.script[0]')));
				} else if(value.toString().indexOf('.') != -1 && value.toString().split(".")[1].length > 4) {
					return callback(new Error(_this.$t('sell.script[1]')));
				} else {
					callback();
				}
			...
			var min_price = (rule, value, callback) => {
				if(!_this.$public.Ints(value)) {
					return callback(new Error(_this.$t('sell.script[2]')));
				} else if(value.toString().indexOf('.') != -1 && value.toString().split(".")[1].length > 2) {
					return callback(new Error(_this.$t('sell.script[3]')));
				} else {
					callback();
				}
			};
...
//			};
			var max_price = (rule, value, callback) => {
				if(!_this.$public.Ints(value)) {
					return callback(new Error(_this.$t('sell.script[4]')));
				} else if(value.toString().indexOf('.') != -1 && value.toString().split(".")[1].length > 2) {
					return callback(new Error(_this.$t('sell.script[5]')));
				} else {
					callback();
				}
			};
			var price = (rule, value, callback) => {
				if(!_this.$public.Ints(value)) {
					return callback(new Error(_this.$t('sell.script[6]')));
				} else if(value.toString().indexOf('.') != -1 && value.toString().split(".")[1].length > 2) {
					return callback(new Error(_this.$t('sell.script[7]')));
				} else {
					callback();
				}
			};
		...
		methods: {
			submitForm(formName) {
				...
				_this.$refs[formName].validate((valid) => {
					if(valid) {
						if(_this.sellData.min_price > _this.sellData.max_price) {                      //最小值不能大于最大值
							_this.$public.msg(_this.$t('sell.script[8]'), 'warning', _this);
							return false
						} else if(_this.sellData.pay_wx == 0 && _this.sellData.pay_backcard == 0 && _this.sellData.pay_alipay == 0) {    //是否有支付方式
							_this.$public.msg(_this.$t('sell.script[9]'), 'warning', _this);
							return false
						} else if(_this.sellData.min_price < _this.sellData.price) {      //不能大于单价
							_this.$public.msg(_this.$t('sell.script[10]'), 'warning', _this);
							return false;
						} else if(Number(_this.sellData.trans_num) > Number(_this.wallone)) {   //用户余额不能大于自己拥有的币
							_this.$public.msg(_this.$t('sell.script[11]'), 'warning', _this);
							_this.dataNull();
							return false;
						} else if(Number(_this.sellData.price) * Number(_this.sellData.trans_num) < Number(_this.sellData.min_price)) {
							//单价*数量，不能小于最小价格
							_this.$public.msg(_this.$t('sell.script[12]'), 'warning', _this);
			...
			pmmaRev(id) {
				var _this = this;
				_this.$confirm(_this.$t('sell.script[13]'), _this.$t('sell.script[14]'), {
					confirmButtonText: _this.$t('sell.script[15]'),
					cancelButtonText: _this.$t('sell.script[16]'),
					type: 'warning'
				}).then(() => {
					...
					_this.$message({
						type: 'info',
						message: _this.$t('sell.script[17]')
					});
				});
			...
			get_price() {
				...
				_this.$http.get(_this.$http.get_price,{params:{}}).then(function(response) {
					if(response.data.status == "200") {
						_this.sellData.price =response.data.data.price;
					} else {
						_this.$public.msg(response.data.msg, 'warning', _this);
					}
				}).catch(function(error) {});
