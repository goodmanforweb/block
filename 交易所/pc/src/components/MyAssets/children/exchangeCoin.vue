...
<template>
  <div class="exchangeCoin">
    <div class="wrap">
      <table></table>
      <h3 class="exchange_title">{{$t('BITPUIBi.dh[0]')}} >{{$t('exchangeCoin.main1[0]')}}</h3>
      <!-- 兑换 -->
        ...
        <el-form :label-position="labelPosition" label-width="80px" :model="sendData" class="exchangeForm">
          ...
          <div>
            <el-form-item class="numleft" :label="$t('exchangeCoin.main3[3]')">
              <el-input v-model="sendData.num" type="num" :placeholder="$t('BITPUIBi.dh[3]')"></el-input>
              <p>{{$route.query.pname}}</p>
              <!-- <p class="ziChan">{{$t('exchangeCoin.main3[0]')}}：{{(sendData.num * ex_rate).toFixed(8)}} {{value}}</p> -->
            </el-form-item>
            <div class="numright">
              <span>{{$t('BITPUIBi.dh[1]')}}</span>
            </div>
          </div>
          ...
          <el-form-item :label="$t('BITPUIBi.dh[2]')">
