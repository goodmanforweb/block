	...
	<div class="exchange">
		<div class="exchange_flex">
			...
			<div class="hangqing flex v">
				<div class="hybar flex v hc wcr">
					<div class="hybar-tit">{{$t('hy[0]')}}</div>
					<div class="hybar-value fw f20">{{balanceShow}}</div>
					<div class="hybar-unit">USDT</div>
					<div class="btn-eye" :class="{active: eyeActive}" @click="handleEye"></div>
				</div>
				<div class="hangqing_nav">
					<div class="item fw">{{$t('hy[2]')}}</div>
				</div>
			...
			<div class="flex1 chart" ref="chart">
				...
				<section class="HistoricalEntrustment">
					<div>
						...
						<ggOrder :changeactive="updateOrder" :part="true" @changeData="listenToMyBoy" ref="offTime1"
							:price="marketPurchase.PriceData" :Code="marketPurchase.code">
						</ggOrder>
					</div>
				</section>
			</div>
			<div class="makeAbargindiv flex v">
					...
					<div class="makeAbarginItem makeAbarginLeft">
						<div class="deepMerge">
							...
							<el-select v-model="deepMergeModel" placeholder="请选择">
								<el-option v-for="item in deepMerge" :key="item.val" :label="item.label"
									:value="item.val">
								</el-option>
							</el-select>
						</div>
										...
										<div class="relativebs">
											<div class="new_price">
												...
												<span class="cnyprice rise"
													v-if="Marketdataheader.change>0">{{$public.toLowFixed(Marketdataheader.price,Marketdataheader.code)}}<i>↑</i></span>
												<span class="cnyprice fall"
													v-else>{{$public.toLowFixed(Marketdataheader.price,Marketdataheader.code)}}<i>↓</i></span>
												<span
											...
											<div class="buyselldiv buyselldivsell">
												<ul class="SelloutData buysell">
													<li class="PurchaseData_list"
														...
														<p :style="`width:`+renderBgWdith(PurchaseData.sell, item.totalSize)+'%'"
															class="sellBg"></p>
														<p class="PurchaseData_name fall">
											...
											<div class="buyselldiv buyselldivbuy">
												<ul class="PurchaseData buysell">
													<li class="PurchaseData_list "
														...
														<p :style="`width:`+renderBgWdith(PurchaseData.buy, item.totalSize)+'%'"
															class="buyBg"></p>
				...
				<section class="transaction  flex1">
...
<script>
	...
	export default {
		...
		methods: {
			getBalance() {
				this.$http.get(this.$http.get_balance, {params: {}}).then(res => {
					if(res.data.status == 200) {
						this.balance = res.data.data.balance;
						const t = sessionStorage.getItem('balanceShow');
						if(t == 'true') {
							this.balanceShow = this.balance;
						}
						else {
							this.balanceShow = '*****';
						}
			...
			setChartWidth() {
				this.$refs.chart.style.width = (window.innerWidth - 350 - 596) + 'px'
			...
			handleEye() {
				...
				this.balanceShow = this.eyeActive ? this.balance : '*****';
				sessionStorage.setItem('balanceShow', this.eyeActive);
			},
			transDeepMerge(val) {
